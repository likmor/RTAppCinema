...
import { Message, UserInfoModel } from "./types/types";
...
interface Props {
  ...
  chatName: string;
  users: UserInfoModel[];
  admin: UserInfoModel | undefined;
}


const Chat: React.FC<Props> = ({ invoke, messages, chatName, users, admin }) => {
  ...
  return (
    ...
    >
      ...
      <Flex direction="row" className="pt-0 justify-center gap-4 ">
        <button
          className={`p-1 w-1/3 rounded-xl ${activeChat
